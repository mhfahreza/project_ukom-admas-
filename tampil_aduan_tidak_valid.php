<?php
        ...
        include "function_tanggal.php";
            ...
            if($level == 'masyarakat')
            {
                $sql_peng_all = "SELECT * FROM pengaduan WHERE nik = '$nik_login' ";
                ...
                $sql_peng_tdk_valid= "SELECT * FROM pengaduan WHERE nik = '$nik_login' and status = 'tidak valid' order by id_pengaduan DESC";
                $eks_peng_tdk_valid= mysqli_query($koneksi, $sql_peng_tdk_valid);
                $jml_peng_tdk_valid= mysqli_num_rows($eks_peng_tdk_valid);
            }
            else
            {
                $sql_peng_all = "SELECT * FROM pengaduan ";
                $eks_peng_all = mysqli_query($koneksi, $sql_peng_all);
                $jml_peng_all = mysqli_num_rows($eks_peng_all);
                ...
                $sql_peng_tdk_valid = "SELECT * FROM pengaduan WHERE status = 'tidak valid' order by id_pengaduan DESC";
                $eks_peng_tdk_valid= mysqli_query($koneksi, $sql_peng_tdk_valid);
                $jml_peng_tdk_valid= mysqli_num_rows($eks_peng_tdk_valid);
            }
?>
...
<div class="container-fluid">
    ...
    <div class="container bg-secondary rounded px-3 pt-3 pb-1 my-3">
        <div class="row d-flex justify-content-around">
            ...
            <div class="card text-bg-danger mb-3 text-center" style="max-width: 18rem;">
                <div class="card-header">Aduan Tidak valid</div>
                <div class="card-body">
                    <h3 class="card-title"><?php echo $jml_peng_tdk_valid; ?></h3>
                </div>
            </div>
    ...
    <div class="container bg-secondary rounded px-3 py-4 my-3">
        ...
        <div class="mb-3 bg-light py-2 rounded px-2">
            <a href="report_tidak_valid.php" class="btn btn-success">XLS</a>
        </div>
        <?php
         }
        ?>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td><a class="btn btn-sm btn-danger"
                                        href="hapus_aduan.php?id_pengaduan=<?php echo $data['id_pengaduan']; ?>"><i
